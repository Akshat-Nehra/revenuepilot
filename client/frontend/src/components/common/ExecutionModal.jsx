...
import { X, Sparkles, ExternalLink, Copy, Check, Loader2, ArrowRight, ShieldCheck, CheckCircle2 } from 'lucide-react';
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/85 backdrop-blur-md animate-fade-in">
      <div className="relative w-full max-w-lg overflow-hidden rounded-2xl border border-slate-800 bg-[#0B0F19] p-6 shadow-2xl shadow-indigo-950/50">
        ...
        {!result ? (
          ...
          /* Result Screen (Payment Link Created) */
          <div className="mt-5 space-y-4">
            ...
            <div className="rounded-xl border border-slate-800 bg-slate-900/80 p-4 space-y-3">
              <div className="flex items-center justify-between text-xs text-slate-400">
                ...
                <span className="font-mono text-indigo-400">{result.paymentLinkId || result.razorpayPaymentLinkId || 'Live'}</span>
              </div>

              {(() => {
                const paymentUrl = result.paymentLink || result.paymentLinkUrl || result.short_url || result.razorpayUrl || '';
                return (
                  <>
                    <div className="flex items-center gap-2 rounded-lg bg-slate-950 p-2.5 border border-slate-800 text-xs font-mono text-slate-200">
                      <span className="truncate flex-1 select-all">{paymentUrl}</span>
                      <button
